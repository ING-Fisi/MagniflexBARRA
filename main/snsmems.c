...
char mems_name_str[SENSOR_NUM][10] = {
	"LIS2DW12", "HTS221", "LIS2MDL", "LSM6DS3", "STTS22H", "LSM6DSL",
};

#ifdef GET_RAWDATA
// I2C data buffer
IRAM_ATTR uint8_t
	i2c_buffered_data[6000 * sizeof(float)]; // TODO: disable if not connected
											 // to MQTT or BT enable.
#endif
...
// In buffer indexes
int azh_in[NSNS], azl_in[NSNS], gxh_in[NSNS], gxl_in[NSNS], gyh_in[NSNS],
	gyl_in[NSNS];
// In buffer indexes
int azh_n[NSNS], azl_n[NSNS], gxh_n[NSNS], gxl_n[NSNS], gyh_n[NSNS],
	gyl_n[NSNS];
...
int snsmems_initilaize(snsmems_t *sns) {
	int nsns = 0;
	set_gpio_output(SNSMEMS_EN_CMD, GPIO_PULLUP_DISABLE, GPIO_PULLDOWN_DISABLE,
					GPIO_INTR_DISABLE);
	...
	u32 buf;
	while (a < 100) {
		if (snsmems_rd(a, SNSMEMS_REG(Status_REG), (u8 *)&buf, REG_LEN) ==
			ESP_OK) {
			//			snsmems_print_stat(buf);
			//			snsmems_print_ver(a);
			sns[nsns].indx =
				a; // assign SNSMEMS I2C address to buffer location.
			nsns++;
		}
		a++;
...
void snsmems_en_cmd(uint32_t en) { gpio_set_level(SNSMEMS_EN_CMD, en); }
...
void snsmems_print_ver(u16 addr) {
	u32 buf;
	if (snsmems_rd(addr, SNSMEMS_REG(Version_REG), (u8 *)&buf, REG_LEN) !=
		ESP_OK) {
		ESP_LOGE(TAG, "error: read SNSMEMS version at address %x(%d).", addr,
				 addr);
		return;
	}
...
/* Porting of Octave script for SNSMEMS data acquisition, encapsulated into a
 * while cycle 	* with a state machine in order to continuously produce new
 * data. 							* Get an index buffer as parameter, or a
 * magniflex_dev_t.									*/
int acq_snsmems_raw_data(magniflex_reg_t *dev) {
	...
	*lendata = (u16)blen;
	...
	for (int i = 0; i < dev->cnt_nsns; i++) {
		rtbuff[2] =
			(dev->snsmems[i].indx == magindx) ? 16 : 0; // Get 16 MAG data.
		if (i2c_master_write_slave_reg(I2C_PORT_NUM, dev->snsmems[i].indx,
									   6 * 4, rtbuff, 4) != ESP_OK) {
			ESP_LOGE(TAG, "error: i2c write fail. [device %d]",
					 dev->snsmems[i].indx);
...
#endif
	rtbuff[0] = 1;
	if (i2c_master_write_slave_reg(I2C_PORT_NUM, 0, 7 * 4, rtbuff, 4) !=
		ESP_OK) {
	...
	while ((long)(T_US - tmt) < (long)(wait * SEC)) {
		//		ESP_LOGI(TAG,"wait for acquisition: %d%%", (int) ((float)
		// j/wait*100) ); 		for ( int i = 0 ; i < dev->cnt_nsns ; i++ ) {
		// if ( i2c_master_read_slave_reg(I2C_PORT_NUM, dev->snsmems[i], (8*4),
		// rtbuff, 4) != ESP_OK ) { 				ESP_LOGE(TAG,"error: i2c
		// write fail. [device %d]", dev->snsmems[i]);
		//			}
		//			ESP_LOGD(TAG,"ret [%d]: %d %d %d %d", i, rtbuff[0],
		// rtbuff[1], rtbuff[2], rtbuff[3] );
		//		}
	...
	while (1) {
		...
		err_cnt = 0;
		for (int i = 0; i < dev->cnt_nsns; i++) {
			if (i2c_master_read_slave_reg(I2C_PORT_NUM, dev->snsmems[i].indx,
										  (8 * 4), rtbuff, 4) != ESP_OK) {
				ESP_LOGE(TAG, "error: i2c write fail. [device %d]",
						 dev->snsmems[i].indx);
				dev->snsmems[i].iscomm = 0;
			...
			ESP_LOGV(TAG, "ret [%d]: %d %d %d %d", dev->snsmems[i].indx,
					 rtbuff[0], rtbuff[1], rtbuff[2], rtbuff[3]);
			if ((rtbuff[0] & 1) == 0) {
		...
		if (chck_time_int(&tmt_wsns, 5) == 1) {
			ESP_LOGW(TAG, "snsmems %d data acuqisition timeout.",
					 dev->snsmems[chck].indx);
			break;
		}
		vTaskDelay(100);
	}
	...
	err_cnt = 0;
	for (int i = 0; i < dev->cnt_nsns; i++) {
		if (i2c_master_read_slave_reg(I2C_PORT_NUM, dev->snsmems[i], BUF_ADDR,
									  (uint8_t *)i2c_buffered_data,
									  blen * 4) != ESP_OK) {
...
void acq_snsmems_env_data(magniflex_reg_t *dev) {
	...
	for (int i = 0; i < dev->cnt_nsns; i++) {
		...
		// riporto l'angolo tra -pi/2 e pi/2 nel caso in cui il materasso sia
		...
		ESP_LOGI(
			TAG,
			"[%d] angl:%.02f|acc:%d,%d,%d|temp:%.01f|hum:%.01f|mag:%d,%d,%d",
			dev->snsmems[i].indx, angle[i], accx[i], accy[i], accz[i], temp[i],
			hum[i], magx[i], magy[i], magz[i]);

		// Assign BODY_P parameter for each current SNSMEMS.
		// ------------------------------------------------.
		if (angle[i] <
			dev->params[BODY_P].val.fbuf[NSNS * i]) { // Check for MIN
			ESP_LOGD(TAG, "BODY_P value MIN found.");
			dev->params[BODY_P].val.fbuf[NSNS * i] = angle[i];
		}
		if (angle[i] >
			dev->params[BODY_P].val.fbuf[NSNS * i + 2]) { // Check for MAX.
		...
		// ----------------------------------------------.
		//		if (  temp[i] < dev->params[TEMP].val.fbuf[NSNS*i] ) { // Check
		// for MIN 			ESP_LOGD(TAG,"TEMP value MIN found.");
		//			dev->params[TEMP].val.fbuf[NSNS*i] = temp[i];
		//		}
		//		if ( temp[i] > dev->params[TEMP].val.fbuf[NSNS*i+2] ) { // Check
		// for MAX. 			ESP_LOGD(TAG,"TEMP value MAX found.");
		//			dev->params[TEMP].val.fbuf[NSNS*i+2] = temp[i];
		//		}
		//		dev->params[TEMP].val.fbuf[NSNS*i+1] = temp[i]; // Assign AVG.
		tmp_temp += temp[i] / (dev->cnt_nsns);

		if ((i + 1) == magindx) { // Only for MAG equipped SNSMEMS. Should be
								  // central on strips.
			// FIXME: For this release humidity will be acquired only from MAG
			// equipped device.
			...
			//--- Heading calculation
			float heading =
				atan2f((float)magy[i], (float)magx[i]) * (180.0f / 3.14f);
			if (heading < 0) {
				heading += 360.0f;
			}
	...
	if ((dev->presence == 0)) {
		for (int e = 0; e < dev->cnt_nsns; e++) {
			if ((dev->snsmems[e].iscomm = 1) &&
				((angle[e] <= dev->prsnc_trsh[e * 2]) ||
				 (angle[e] >= dev->prsnc_trsh[e * 2 + 1]))) {
				ESP_LOGW(
					TAG,
					"Presence for threshold --------------------------------> "
					"%d  [%.02f] [%.02f] [%.02f]",
					e, angle[e], dev->prsnc_trsh[e * 2],
					dev->prsnc_trsh[e * 2 + 1]);
				dev->presence = 1;
				// rgbled_set_c_presence(0x050000, dev->presence);
				init_cirf(&bpm_filt);
				if (dev->start_sleep_t == 0) {
					dev->start_sleep_t =
						get_curtimestamp(); // Get time sleep start.
		...
		for (int e = 0; e < dev->cnt_nsns; e++) {
			...
			if ((dev->snsmems[e].iscomm = 1) &&
				(angle[e] > dev->prsnc_trsh[e * 2]) &&
				(angle[e] < dev->prsnc_trsh[e * 2 + 1])) {
				ESP_LOGW(TAG, "Threshold[%d]: %.2f, %.2f, %.2f.", e, angle[e],
						 dev->prsnc_trsh[e * 2], dev->prsnc_trsh[e * 2 + 1]);
				chck++;
			} else {
				ESP_LOGE(TAG, "Threshold[%d]: %.2f, %.2f, %.2f.", e, angle[e],
						 dev->prsnc_trsh[e * 2], dev->prsnc_trsh[e * 2 + 1]);
			}
		}
...
int get_periods_data(magniflex_reg_t *dev, int i, uint16_t *buf, int len,
	...
	if (len > 0) {
		...
		i2c_master_read_slave_reg(I2C_PORT_NUM, dev->snsmems[i].indx, addr,
								  (uint8_t *)buf, 4 * nread);
...
bpm_data_t sel_best_bpm(bpm_data_t *bpm) {
	...
	for (int i = 0; i < 6; i++) {
		if ((bpm[i].k1 < selbpm.k1) && (bpm[i].k1 != NAN) &&
			(bpm[i].k2 != NAN)) {
	...
	ESP_LOGI(TAG,
			 ...
			 "\t\tR: \t|%.02f\t|\n"
			 "\t\tdif:\t|%.02f\t|\n",
			 selbpm.max, selbpm.avg, selbpm.min, selbpm.k1, selbpm.k2,
			 selbpm.ratio, fabsf(selbpm.k1 - selbpm.k2));
...
void print_bpm_vectors(bpm_data_t *bpm) {
	...
	ESP_LOGI(TAG, "max:\t|%.02f\t|%.02f\t|%.02f\t|%.02f\t|%.02f\t|%.02f\t|",
			 bpm[0].max, bpm[1].max, bpm[2].max, bpm[3].max, bpm[4].max,
			 bpm[5].max);
	ESP_LOGI(TAG, "avg:\t|%.02f\t|%.02f\t|%.02f\t|%.02f\t|%.02f\t|%.02f\t|",
			 bpm[0].avg, bpm[1].avg, bpm[2].avg, bpm[3].avg, bpm[4].avg,
			 bpm[5].avg);
	ESP_LOGI(TAG, "min:\t|%.02f\t|%.02f\t|%.02f\t|%.02f\t|%.02f\t|%.02f\t|",
			 bpm[0].min, bpm[1].min, bpm[2].min, bpm[3].min, bpm[4].min,
			 bpm[5].min);
	...
	ESP_LOGI(TAG, "R:\t|%.02f\t|%.02f\t|%.02f\t|%.02f\t|%.02f\t|%.02f\t|",
			 bpm[0].ratio, bpm[1].ratio, bpm[2].ratio, bpm[3].ratio,
			 bpm[4].ratio, bpm[5].ratio);
	ESP_LOGI(TAG, "dif:\t|%.02f\t|%.02f\t|%.02f\t|%.02f\t|%.02f\t|%.02f\t|",
			 fabsf(bpm[0].k2 - bpm[0].k1), fabsf(bpm[1].k2 - bpm[1].k1),
			 fabsf(bpm[2].k2 - bpm[2].k1), fabsf(bpm[3].k2 - bpm[3].k1),
			 fabsf(bpm[4].k2 - bpm[4].k1), fabsf(bpm[5].k2 - bpm[5].k1));
}
...
int acq_snsmems_data(magniflex_reg_t *dev) {
	...
	best_bpm.max = 0.0f;
	best_bpm.k1 =
		10000.0f; // High value because it has to be as low as possible.
	best_bpm.k2 = 10000.0f;

	if (magindx ==
		0) { // If magindx already assigned, device are already initialized.

		// 1. SENSORS STATUS
		// ------------------------------------------------------------------
		ESP_LOGI(TAG, "Read status registers.");
		for (int i = 0; i < dev->cnt_nsns; i++) {
			if (i2c_master_read_slave_reg(I2C_PORT_NUM, dev->snsmems[i].indx, 0,
										  rtbuff, 4 * 2) != ESP_OK) {
				ESP_LOGE(TAG, "error: i2c write fail. [device %d]",
						 dev->snsmems[i].indx);
				dev->snsmems[i].iscomm = 0;
	...
	for (int i = 0; i < dev->cnt_nsns; i++) {
		if (i2c_master_read_slave_reg(I2C_PORT_NUM, dev->snsmems[i].indx,
									  OFFS_AZ_CNT * 4, &raw_periods[i][0],
									  4 * 3) != ESP_OK) {
			ESP_LOGE(TAG, "error: i2c read fail. [device %d]",
					 dev->snsmems[i].indx);
			dev->snsmems[i].iscomm = 0;
		}
		dev->snsmems[i].iscomm = 1;
	...
	for (int i = 0; i < dev->cnt_nsns; i++) {
		if (i2c_master_read_slave_reg(I2C_PORT_NUM, dev->snsmems[i].indx,
									  OFFS_AZ_MAX * 4, (uint8_t *)raw_minmax[i],
									  4 * 6) != ESP_OK) {
			ESP_LOGE(TAG, "error: i2c read fail. [device %d]",
					 dev->snsmems[i].indx);
			dev->snsmems[i].iscomm = 0;
	...
	for (int i = 0; i < dev->cnt_nsns; i++) {
		...
		if (dev->snsmems[i].iscomm == 0) {
			ESP_LOGW(TAG, "SNSMEMS %d didn't replayed, goes to next iteration.",
					 dev->snsmems[i].indx);
			continue;
		}

		//		ESP_LOGI(TAG, "------------------- board %d
		//----------------------", dev->snsmems[i].indx); 		for ( int j = 0
		//; j < 6 ; j++ ) { // DBG. 			ESP_LOGI(TAG, "period[%d][%d] =
		// %d.", i, j, periods[i][j]);
		//		}
		...
		if ((dev->presence == 0) ||
			...
			//			ESP_LOGW(TAG,"No Heart Rate detected device %d",
			// dev->snsmems[i]);
			// If last device and the previous didn't assigned best_bpm, return
			// and don't assign new values.
	...
	//			"\t\tR:\t|%.02f\t|\n"
	//			"\t\tdif:\t|%.02f\t|\n",
	//			best_bpm_dev, best_bpm.max, best_bpm.avg, best_bpm.min,
	// best_bpm.k1, best_bpm.k2, best_bpm.ratio, fabsf(best_bpm.k1-best_bpm.k2)
	//);

	// Assign HEART parameter for each SNSMEMS.
	// ----------------------------------------
	//	if (  best_bpm.avg < dev->params[HEART_R].val.fbuf[0] ) { // Check for
	// MIN
	////		ESP_LOGD(TAG,"Position value MIN found.");
	//		dev->params[HEART_R].val.fbuf[0] = best_bpm.avg;
	//	}
	//	if ( best_bpm.avg > dev->params[HEART_R].val.fbuf[2] ) { // Check for
	// MAX.
	...
	//	push_circf_val(&bpm_filt,
	//(best_bpm.avg+best_bpm.min-best_bpm.k1*best_bpm.min)/2);
	dev->params[HEART_R].val.fbuf[1] = get_circf_val(&bpm_filt);
#else
	dev->params[HEART_R].val.fbuf[1] =
		(best_bpm.avg + best_bpm.min) / 2; // Assign AVG+MIN/2.
#endif
	...
	// -------------------------------------------
	// TODO: For now get from breath division with randomized denominator [5 :
	// 7].
	if ((best_bpm.avg / (5 + rand_int_decimal(2, 1))) <
		dev->params[BREATH_R].val.fbuf[0]) { // Check for MIN
		ESP_LOGD(TAG, "Position value MIN found.");
		dev->params[BREATH_R].val.fbuf[0] =
			(best_bpm.avg / (5 + rand_int_decimal(2, 1)));
	}
	if ((best_bpm.avg / (5 + rand_int_decimal(2, 1))) >
		dev->params[BREATH_R].val.fbuf[2]) { // Check for MAX.
		ESP_LOGD(TAG, "Position value MAX found.");
		dev->params[BREATH_R].val.fbuf[2] =
			(best_bpm.avg / (5 + rand_int_decimal(2, 1)));
	}
...
void snsmems_acq_en(u8 val) { en_acq_snsmemes = val; }

u8 snsmems_get_acq_en(void) { return en_acq_snsmemes; }
...
int snsmems_nvs_get_thrsh(float *thresh) {
	...
	if (err != ESP_OK) {
		...
		if (required_size == 0) {
			...
			err = nvs_get_blob(nvsh, THRSH_BLOBS_TAG, thresh,
							   &required_size); // node structure
			if (err != ESP_OK) {
				ESP_LOGD(TAG, "nvs_get_blob: %s", esp_err_to_name(err));
				return err;
			}
...
long snsmems_get_wdt(void) { return (long)(T_US - t_snsmems_wdt); }
